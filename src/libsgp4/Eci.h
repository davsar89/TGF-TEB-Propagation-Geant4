...
public:
     ...
     */
    Eci(const sgp4::DateTime &dt, const CoordGeodetic &geo) {
        ToEci(dt, geo);
    }
     ...
     */
    Eci(const sgp4::DateTime &dt, const Vector &position)
            : m_dt(dt), m_position(position) {
    }
     ...
     */
    Eci(const sgp4::DateTime &dt, const Vector &position, const Vector &velocity)
     ...
     */
    bool operator==(const sgp4::DateTime &dt) const {
        return m_dt == dt;
    }
     ...
     */
    void Update(const sgp4::DateTime &dt, const CoordGeodetic &geo) {
        ToEci(dt, geo);
    }
     ...
     * @returns the date
     */
    sgp4::DateTime GetDateTime() const {
        return m_dt;
    }
...
private:
    void ToEci(const sgp4::DateTime &dt, const CoordGeodetic &geo);
